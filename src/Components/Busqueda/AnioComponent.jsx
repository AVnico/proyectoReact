...
export function AnioComponent() {
    ...
    useEffect(() => {
        const fetchPeliculas = async () => {
            try {
                const response = await axios.get(`http://localhost:5000/api/peliculas/anio/${anio}`);
                setPeliculas(response.data);
            } catch (err) {
                setError('No se pudieron cargar las películas para este año');
                console.error(err);
            }
        };
        fetchPeliculas();
